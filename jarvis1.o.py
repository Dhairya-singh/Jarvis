...
def me():
    s = "hello sir Im fridaye 2. point o " \
        "Created by Dhairya singh"
...
def wishme(s):
    ...
    if (hour >= 0 and hour < 12):
        speak(f"Good Monring {s}!")

    elif hour >= 12 and hour < 18:
        speak(f"Good After Noon {s}!")
...
def opendetiles():
    speak("you want to only see the user deties or i should speack")
...
def takecommend():
    ...
    return query
def stoplisting():
    speak("for how much seconed  you want to stop fridaye  from listening commands")
    try:
        a = int(takecommend())
...
def walkupme():
    ...
    speak(f"it is{strftime} you have to walkup")

def news(str):
  global i
  if i == 0:
      speak(f"ofcures {str} which news  you want to listen")
  else:
...
def friday():
    ...
    if j==0:
        ...
        speak("friday in a sleep mode sir")
    try:
    ...
    while (1):
        ...
        try:
            ...
            if 'Dhairya' in s1:
                ...
                speak("plesae identifai your self Are you a male of female  !")
     ...
     while (1):
        ...
        try:
            ...
            if 'wikipedia' in queuery:
                ...
                speak(f"opening google{sex} !")

            elif 'the time' in queuery:
                strtime = datetime.datetime.now().strftime("%H:""%M:""%S")
                speak(f"{sex} the time is{strtime}")

            elif 'open code' in queuery:

                codepath = "C:\\Users\\Dhairya singh\\AppData\\Local\\Programs\\Microsoft VS Code\\Code.exe"
                os.startfile(codepath)
                speak(f"opening vs code {sex}!")
                speak(
                ...
                me()
            elif  "will you be my friend" in queuery:
                ...
                speak("I have been created by Dhairya.")
                ...
                speak(f"ofcurse {sex} ! how can i help yous {sex}")
                speak(f'question {sex}!')
                s = takecommend()
                print(s)
                speak(
                    "There are 3 thing that i can do for you sir i can search for it on google or youtube or wikipedia")
                speak(f"where i should to serach {sex}")
                s1 = takecommend().lower()
                if s1 == 'google':
                    speak(f"opening  google {sex}!")
                    webbrowser.open(
                ...
                music_dir = "C:\\Users\\Dhairya singh\\Desktop\\my"
                songs = os.listdir(music_dir)
                print(songs)
                random = os.startfile(os.path.join(music_dir, songs[1]))

            elif 'wrok' in queuery:
                speak(f"sorry {sex} because of me you face some problem "
                ...
                speak(f"hello {sex} !what is your friends name ")
                s = takecommend().lower()
                speak(f"nice to meet you {s} ! have a good day!")

            elif "know Dhairya" in queuery:
                speak(
                    f"yes{sex} i know him becuase of him im able to talk to you he is a very nice person would you like to know more about mister Dhairya singh")
                s = takecommend().lower()
                if s == "yes":
                    me1()
                else:
                ...
                speak("wow why did you not tell me before that hy babe how are you will you be my gf")
            ...
            elif 'friday' in queuery:
                speak(f"fridaye 2 point 0 in your service {sex}")
            elif 'open python'in queuery:
            ...
            print(s)
            speak(f" i dont understand sir please give me some other commend {sex}")
 ...
 while True:
  try:
    ...
    if 'yes' in s1:
      ...
      speak("i could not understand sir plesase speack again")
